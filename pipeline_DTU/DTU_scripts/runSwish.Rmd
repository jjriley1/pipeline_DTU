...
all_foldernames = list.files(quant_dir)
all_filenames = list.files(quant_dir, pattern="*.sf")
all_foldernames = all_foldernames[!(all_foldernames %in% all_filenames)]

#strip file name and apply to each item in the list
files<-file.path(quant_dir, all_foldernames, all_filenames) 
sample_names = sapply(all_foldernames, function(x) substr(x, 0, nchar(x)-3))
sample_names = sapply(all_foldernames, function(x) strsplit(x, "[.]")[[1]][1])
...
```{r filter_sample_table_and_factorize}
sample_table = sample_table %>% dplyr::filter(var1 %in% filter_by_var1) %>% mutate(var1 = as.factor(var1))

if (num_vars==2){
  sample_table = sample_table %>% dplyr::filter(var2 %in% filter_by_var2) %>% mutate(var2 = as.factor(var2))
}
```
